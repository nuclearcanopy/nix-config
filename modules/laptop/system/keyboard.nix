...
# 40ms keyboard debounce via interception-tools.
# Filters chatter: if the same key is pressed again within 40ms of release, ignore it.
...
in
{
  services.interception-tools = {
    enable = true;
    udevmonConfig = ''
      - JOB: "${pkgs.interception-tools}/bin/intercept -g $DEVNODE | ${evdev-debounce}/bin/evdev-debounce 40 | ${pkgs.interception-tools}/bin/uinput -d $DEVNODE"
        DEVICE:
          NAME: ".*[Kk]eyboard.*"
    '';
  };
